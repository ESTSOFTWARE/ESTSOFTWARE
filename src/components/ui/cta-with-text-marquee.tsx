...
export default function CTAWithVerticalMarquee({
  ...
  description = "Aplicaciones web, móviles y sistemas a la medida. Nos encargamos del diseño, el desarrollo y el mantenimiento para que tú te concentres en crecer.",
  primaryLabel = "SOLICITAR COTIZACIÓN",
  primaryHref = "#contacto",
  secondaryLabel = "AGENDAR LLAMADA DE 15 MIN",
  ...
  return (
    <div className={cn("min-h-screen bg-background text-foreground flex items-center justify-center px-6 py-12 overflow-hidden", className)}>
      <div className="w-full max-w-7xl animate-fade-in-up">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
          {/* Contenido izquierdo */}
          <div className="space-y-8 max-w-xl">
            <h2 className="text-5xl md:text-6xl lg:text-7xl font-medium leading-tight tracking-tight text-balance text-foreground animate-fade-in-up [animation-delay:200ms]">
              {title}
            </h2>
            <p className="text-lg md:text-xl text-muted-foreground leading-relaxed text-pretty animate-fade-in-up [animation-delay:400ms]">
              {description}
            </p>
            <div className="flex flex-wrap gap-4 animate-fade-in-up [animation-delay:600ms]">
              <a href={primaryHref} className="group relative inline-flex px-6 py-3 bg-foreground text-background rounded-md font-medium overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-lg">
                <span className="relative z-10">{primaryLabel}</span>
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-200%] group-hover:translate-x-[200%] transition-transform duration-700"></div>
              </a>
              <a href={secondaryHref} className="group relative inline-flex px-6 py-3 bg-secondary text-secondary-foreground rounded-md font-medium overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-lg border border-border">
